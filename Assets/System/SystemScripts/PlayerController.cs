...
public class PlayerController : MonoBehaviour
{
    ...
    */
    ...
    private void DoChargeJump()
    {
        ...
        chargePct = Mathf.Max(chargePct, minCharge);

        float finalJumpForce = currentJumpValues.jumpForce * chargePct;

        rigidbody2D.gravityScale = currentJumpValues.jumpGravity;
        rigidbody2D.linearVelocityY = 0; 

        rigidbody2D.AddForce(new Vector3(0, finalJumpForce, 0));

        float horizontalBoost = Input.GetAxisRaw("Horizontal") * (currentMovementState.maxSpeed * 1.5f);
        rigidbody2D.linearVelocityX = horizontalBoost;
